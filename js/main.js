...
// --------sorts by name according to changes in roast
function nameByRoast(roast) {
...
function updateRoast(e) {
    ...
    if (nameSelection.value === "") {
        ...
        nameByRoast(filteredCoffees);
    }
}

// --------updates displayed list according to changes in name--------
function updateName(e) {
    e.preventDefault();
    const nameSelectionNew = nameSelection.value.toLowerCase()
    let filteredCoffees = [];
    let coffeesByRoast = [];
    coffees.forEach(coffee => { // if
        if (coffee.roast === roastSelection.value) {
...
function createNewCoffee(e) {
    ...
    if (roast !== "all" && name !== "") {
        const newCoffee = {
            id: coffees.length + 1, name: name, roast: roast
        }
...
const coffees = [{id: 1, name: 'Light City', roast: 'light'}, {id: 2, name: 'Half City', roast: 'light'}, {
    id: 3,
    name: 'Cinnamon',
    roast: 'light'
